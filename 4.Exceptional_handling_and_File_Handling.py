...
def calculate_square(num):
    if num < 0 or num>20 :
        raise NumberOutOfRangeError("Number should lies between 0 and 20")
    else:
        return num*num

try:
    result = calculate_square(21)
...
# The 'pickle' module bundled with Python's standard library defines functions for serialization (dump() and dumps()) 
# and deserialization (load() and loads()). 
# The data format of pickle module is very Python specific.
...
def num_function(input_num):
    if input_num < number:
        ...
        return ("You are eligiblt to vote")
try:
